...
def main(unused_argv):
  ...
  for pred_dict, image_path in zip(predictions, image_files):
    ...
    crf_argmax = np.expand_dims(np.expand_dims(np.argmax(crf_result, axis=2), axis=0),axis=3)
    crf_decode = np.squeeze(preprocessing.decode_labels(crf_argmax)).transpose((1,0,2))
